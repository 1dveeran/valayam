...
- **AI Sandbox & Deep Analysis**: Leverages local LLMs for intelligent WAF bypass payload generation, decompiles WebAssembly (WASM), and analyzes Source Maps to uncover hidden endpoints and secrets.
- **Enterprise Dependency Auditing**: Connects natively to the offline `vuln-db.sqlite` artifact generated by the Valayam Platform to perform lightning-fast local CVE cross-referencing without external network calls.
