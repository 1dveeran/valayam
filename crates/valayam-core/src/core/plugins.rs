use std::sync::Arc;
use valayam_engine::traits::{ScanPlugin, PluginOutcome};
use valayam_engine::traits::ScanContext;
use valayam_models::templates::schema::{VulnerabilityTemplate, TemplateMetadata};
...
impl ScanPlugin for DnsAuditPlugin {
    ...
    async fn execute(&self, ctx: &ScanContext) -> PluginOutcome {
        ...
        if count > 0 { PluginOutcome::Matched { count } } else { PluginOutcome::NoMatch }
    }
}
...
mod tests {
    use super::*;
    use std::sync::Arc;
    use valayam_models::template::schema::{TemplateInfo, VulnerabilityTemplate};
