...
async fn main() -> anyhow::Result<()> {
    ...
    println!("{} Agent starting — worker_id={}", "[+]".green().bold(), cfg.worker_id);
    println!("{} Platform URL: {}", "[+]".green().bold(), cfg.platform_url);
...
async fn poll_job(client: &reqwest::Client, cfg: &AgentConfig) -> anyhow::Result<Option<AgentJob>> {
    let url = format!("{}/api/v1/jobs/poll", cfg.platform_url.trim_end_matches('/'));
...
async fn execute_and_report(
    ...
    let scan_args = valayam_cli::cli::Args {
        ...
        format: job.config.output_format.clone().unwrap_or_else(|| "json".into()),
    ...
    match client.post(&url).json(&result).send().await {
        Ok(resp) if resp.status().is_success() => {
            println!("{} Results reported for job {}", "[✓]".green().bold(), job.job_id);
        }
        Ok(resp) => {
            tracing::warn!("Result POST returned {} for job {}", resp.status(), job.job_id);
        }
        Err(e) => {
...
async fn send_heartbeat(
    ...
    let heartbeat = AgentHeartbeat {
        ...
        status: if current_job_id.is_some() { "scanning".to_string() } else { "idle".to_string() },
