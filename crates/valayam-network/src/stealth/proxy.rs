...
impl ProxyRotator {
    ...
    pub fn new() -> Self {
        Self {
            state: Arc::new(RwLock::new(ProxyState { proxies: Vec::new(), index: 0 })),
            max_failures: 3,
    ...
    pub async fn random(&self) -> Option<String> {
        ...
        let healthy: Vec<&ProxyEntry> = state.proxies.iter()
                ...
                match client.get(&api_url).send().await {
                    Ok(response) => {
                        if let Ok(text) = response.text().await {
                            ...
                            info!("Successfully updated proxy pool. Total proxies: {}", locked_state.proxies.len());
                        }
                    }
                    Err(e) => {
...
mod tests {
    ...
    async fn test_round_robin() {
        ...
        assert_eq!(rotator.next().await, Some("http://proxy1:8080".to_string())); // wraps
    }
