                    ...
                    if let Ok(body) = resp.text().await {
                        if body.contains("__schema") && body.contains("queryType") {
                            let mut finding = FindingOwned::from_template_and_info(
                                ...
                                template_meta,
                                host.clone(),
                                "GraphQL Introspection query is enabled, exposing API schema.".to_string(),
                            );
                            finding.severity = "Low".to_string(); // Changed to Low, as Introspection itself is just info disclosure
                            let mut results = vec![finding];
                            ...
                            if let Some(alias_payload) = super::mutator::GraphqlMutator::generate_alias_batch_payload(&body, 100) {
                                if let Ok(atk_resp) = req_client.post(reqwest_url.clone()).json(&alias_payload).send().await {
                                    if atk_resp.status().is_server_error() || atk_resp.status().is_success() {
                                        ...
                                        let mut alias_finding = FindingOwned::from_template_and_info(
                                            template_id,
                                            template_meta,
                                            host.clone(),
                                            "GraphQL endpoint allows extensive alias batching (100 aliases), potentially leading to Denial of Service (DoS).".to_string(),
                                        );
                                        alias_finding.template_name = format!("{} - Alias Batching DoS", template_meta.template_name());
                                        alias_finding.severity = "High".to_string();
                                        results.push(alias_finding);
                                    }
                                }
                            }
                            ...
                            if let Ok(atk_resp) = req_client.post(reqwest_url.clone()).json(&circ_payload).send().await {
                                if atk_resp.status().is_server_error() {
                                    let mut circ_finding = FindingOwned::from_template_and_info(
                                        template_id,
                                        template_meta,
                                        host.clone(),
                                        "GraphQL endpoint crashed (HTTP 500) when supplied with circular fragments.".to_string(),
                                    );
                                    circ_finding.template_name = format!("{} - Circular Fragments", template_meta.template_name());
                                    circ_finding.severity = "Critical".to_string();
                                    results.push(circ_finding);
                                }
                            }
