...
RUN mkdir -p /out/plugins-wasm && \
    for plugin in plugins-wasm/*/; do \
        name=$(basename "$plugin"); \
        wasm_file="target/wasm32-unknown-unknown/release/valayam_plugin_${name//-/_}.wasm"; \
        [ -f "$wasm_file" ] && cp "$wasm_file" "/out/plugins-wasm/${name}.wasm"; \
    done && \
    cp target/x86_64-unknown-linux-musl/release/valayam /out/valayam
